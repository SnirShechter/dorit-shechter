<template>
	<div class="flex justify-between text-xl items-center">
		<g-link
			...
			>&larr; <span>הקודם</span></g-link
		>
		<div class="text-base">Page {{ currentPage }} of {{ totalPages }}</div>
		<g-link :to="nextPage" :class="{ 'text-gray-400 hover:text-gray-400 cursor-not-allowed': !showNextPage }"
			><span>הבא</span> &rarr;</g-link
		>
	</div>
</template>
...
export default {
	...
	computed: {
		...
		previousPage() {
			return [0, 1].includes(this.currentPage - 1) ? this.base : `${this.base}/${this.currentPage - 1}`
		},
		showNextPage() {
